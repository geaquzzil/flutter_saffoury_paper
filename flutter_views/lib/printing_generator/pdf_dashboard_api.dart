...
class PdfDashnoardApi<T extends PrintableDashboardInterface,
  ...
  Widget buildInvoiceMainInfoHeader() {
    List<List<InvoiceHeaderTitleAndDescriptionInfo>> inf =
        printObj.getPrintableDashboardHeaderInfo(context, setting);
    return Container(
  ...
  Widget buildInvoiceMainTable() {
    ...
    for (int i = 0; i < details.length; i++) {
      ...
      } else if (element is PrintableCustomFromPDFInterface) {
      } else if (element is PrintableCustomInterface) {}
    }

    checkToSort(headers, list);
  ...
  List<pw.Column> get buildInvoiceBottomInfoWithQrCodeList {
    return [
      Column(
          ...
          // mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: printObj
              .getPrintableDashboardAccountInfoInBottom(context, setting)
              .map((e) =>
  ...
  Widget buildMainTotal() {
    List<InvoiceTotalTitleAndDescriptionInfo> totals =
        printObj.getPrintableDashboardFooterTotal(context, setting);
    List<InvoiceTotalTitleAndDescriptionInfo> totalDes =
        printObj.getPrintableDashboardTotalDescripton(context, setting);
